...
RippleButton {
    ...
    property string filePath: {
        ...
        // If there's a subtitle (folder path), check if rawValue is a full path to an image
        if (subtitle && raw) {
            return Images.isValidImageByName(raw) ? raw : "";
        }
        // Fallback for other cases
        return Images.isValidImageByName(entry?.name) ? entry?.name : "";
    }
    property bool blurImage: entry?.blurImage ?? false
    property string itemSubtitle: entry?.subtitle ?? ""
    ...
    RowLayout {
        ...
        ColumnLayout {
            ...
            StyledText { // Subtitle (e.g., folder path for files)
