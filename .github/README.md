<div align="center">
     <h1>  [ phoenix-dots hyprland ] </h1>
</div>



<div align="center"> 
     <h2>• screenshots •</h2>

| Overall view of widgets | Overlay and AI sidebar |
| ----------- | ----------- |
| <img width="1919" height="1078" alt="image" src="https://github.com/user-attachments/assets/9297bec7-63b4-47bf-8905-9a4baa8de4e9" /> | <img width="1916" height="1078" alt="image" src="https://github.com/user-attachments/assets/53c3b4be-9ba0-40dc-8570-c6a3a80c18cf" /> |  

| Media mode | Wallpaper service and settings |
| ----------- | ----------- |
| <img width="1920" height="1077" alt="image" src="https://github.com/user-attachments/assets/a966c5ca-ef0a-4ecf-882b-e7ef55dde74e" /> | <img width="1920" height="1078" alt="image" src="https://github.com/user-attachments/assets/aa822b97-cc55-46fe-a2e0-3ebdbdb247c6" /> |




</div>

<div align="center">
    <h2>• warning •</h2>
</div>

This is my personal Hyprland dotfiles repository, maintained by **phoenix**.

Based on [illogical-impulse](https://github.com/end-4/dots-hyprland) by end-4, with inspiration from [vaguesyntax/ii-vynx](https://github.com/vaguesyntax/ii-vynx).

This repo contains my personal tweaks and configurations. This is my current daily driver.
However there can (and will) be bugs **so don't expect stability here**. Use these dots if you know what you are doing.

Before saying "these dots are bloated," keep in mind that there's a toggle for everything.

<div align="center">
    <h2>• installation •</h2>
</div>


1. Clone this repository like this:

```
git clone https://github.com/Better-Than-You/phoenix-dots.git --recurse-submodules
```

2. Run the setup script and follow the instructions:

```
./setup-phoenix.sh
```

Tip: You can see all flags you can use with `--help`:
```
./setup-phoenix.sh --help
```

<div align="center">
    <h2>• updating •</h2>
</div>

Run the setup script:

```
./setup-phoenix.sh
```

<div align="center">
    <h2>• documentation •</h2>
</div>

Please refer to [this repository's wiki](https://github.com/Better-Than-You/phoenix-dots/wiki) for detailed component descriptions and further information.

Please refer to [this wiki page about limitations](https://github.com/Better-Than-You/phoenix-dots/wiki/Known-Issues-and-Limitations) to see current bugs, the details of known issues about this repo and limitations of hyprscrolling plugin implementation.


<div align="center">
    <h2>• credits •</h2>
</div>

### **[end-4](https://github.com/end-4):** Creator of illogical-impulse, alien

**[ii](https://github.com/end-4/dots-hyprland):** A perfect hyprland dots in material-3 style

**[vaguesyntax](https://github.com/vaguesyntax):** Creator of [ii-vynx](https://github.com/vaguesyntax/ii-vynx), inspiration for this fork

**[Quickshell](https://quickshell.org/):** Qt-Quick based widget system for hyprland

**[Hyprland](https://hypr.land/):** Loves-to-crash wayland compositor


**Since you scrolled all the way down here, can I get your star? ⭐**
